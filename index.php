		...
		{   
			...
			if(media_storage=="s3")
			 {  
			     ...
			     /********************optimize video size and upload on aws s3**************************/
                    ...
                    if(is_numeric($sound_id))
                    {
                        //merge sound and save video
                            $testSound="upload/audio/".$sound_id.".aac";
                            
                            $withoutSoundOuput="tmp/".$fileName."withOutSound.mp4";
                            $finalOutput= "tmp/".$fileName."withSound.mp4";
                            
                            $without_audio = "$ffmpeg_string -i $resultFile -c copy -an $withoutSoundOuput";
                            exec($without_audio);
                            
                            $cmd_new = "$ffmpeg_string -i $withoutSoundOuput -i $testSound -c:v copy -c:a aac -shortest $finalOutput";
                            exec($cmd_new);
                         
                         //merge sound and save video
                         
                         //upload files on aws s3
                        $video_url=uploadS3($finalOutput,"videos");
                    }
                    else
                    {
                        //upload files on aws s3
                ...
                @unlink("tmp/".$fileName."withOutSound.mp4");
                @unlink("tmp/".$fileName."withSound.mp4");
                @unlink("tmp/".$fileName.".gif");
                @unlink("tmp/".$fileName.".aac");
                @unlink("tmp/".$fileName.".mp3");
                    
                //remove from tmpfile
                @unlink("tmp/tmpfile/".$fileName.".jpg");
                @unlink("tmp/tmpfile/".$fileName."withSound.mp4");
                @unlink("tmp/tmpfile/".$fileName.".gif");
